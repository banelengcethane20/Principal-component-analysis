...
/*********************************************************/
/* PCA: Classical correlation and robust correlation */
/*********************************************************/

/*********************************************************/
/* PCA from PROC PRINCOMP */
/*********************************************************/
proc princomp data=Crime /* add COV option for covariance analysis */
   ...
   id State;
   ods select Eigenvectors EigenValues ScreePlot '2 by 1' '3 by 1';
run;
...
print V[r=varNames c=("Prin1":"Prin7") L="Eigenvectors" format=8.5];
print (scores[1:5,])[c=("Prin1":"Prin7") L="Scores" format=8.5];

/* compare to the results from PROC PRINCOMP */
/*
use PCAStats;
  read all var "_TYPE_" into type;
  read all var varNames into m;
close;
D2 = T( m[ loc(type="EIGENVAL"), ] );
V2 = T( m[ loc(type="SCORE"), ] );
print (max(abs(D - D2)))[L="max diff eigenvalues"];
print (max(abs(V - V2)))[L="max diff eigenvectors"];

use Components;
read all var ("Prin1":"Prin7") into Scores2;
close;
print (max(abs(scores - Scores2)))[L="max diff scores"];
create scoreCorr from scores[c=("PC1":"PC7")];
   append from scores;
close;
*/

/**********************************/
/* a robust PCA: the CORR analysis */
/**********************************/
...
print (D/D[+])[L="Proportion"];
/* to compare scores, reverse some eigenvectors */
/* see  http://blogs.sas.com/content/iml/2017/08/07/eigenvectors-not-unique.html */
scores[,{2 4}] = -scores[,{2 4}];
create scoreRCorr from scores[c=("RobPC1":"RobPC7")];
append from scores;
close;
...
data All;
merge Components_CORR scoreRCorr;
L1 = State;   /* if render by template=Stat.Princomp.Graphics.ScorePlot */
label Prin1 = "Classical Component 1 (58.8%)"
      Prin2 = "Classical Component 2 (17.7%)"
      Prin3 = "Classical Component 3 (10.4%)"
      Prin4 = "Classical Component 4 (4.5%)"
      RobPC1 = "Robust Component 1 (66.7%)"
      RobPC2 = "Robust Component 2 (18.5%)"
      RobPC3 = "Robust Component 3 (7.8%)"
      RobPC4 = "Robust Component 4 (2.6%)";      
run;
/*
proc sgrender data=All template=Stat.Princomp.Graphics.ScorePlot;
dynamic XVAR="RobPC1" YVAR="RobPC2";
run;
proc sgrender data=All template=Stat.Princomp.Graphics.ScorePlot;
dynamic XVAR="RobPC1" YVAR="RobPC3";
run;
*/
proc means data=All;
var Prin1-Prin3 RobPC1-RobPC3;
run;
/* first and second component scores */
title "Classical Scores";
proc sgplot data=All;
format State $7.;
xaxis min=-4.1 max=7.5 grid;
yaxis min=-3 max=2.7 grid; 
...
xaxis min=-4.1 max=7.5 grid;
yaxis min=-3 max=2.7 grid; 
scatter x=RobPC1 y=RobPC2 / datalabel=State;
run;

/* first and third component scores */
title "Classical Scores";
proc sgplot data=All;
format State $7.;
xaxis min=-4.1 max=7.5 grid;
yaxis min=-2.3 max=4.1 grid; 
...
xaxis min=-4.1 max=7.5 grid;
yaxis min=-2.3 max=4.1 grid; 
scatter x=RobPC1 y=RobPC3 / datalabel=State;
run;
